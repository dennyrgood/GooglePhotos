...
class BrowserController:
    ...
    return { x: rect.left + rect.width/2, y: rect.top + rect.height/2 };
            ...
            try:
                ...
                print('[BACKSPACE] textarea became active')
            except Exception:
                print('[BACKSPACE] WARNING: textarea did not become active within timeout')
    ...
    const target = candidates[0];
    const rect = target.rect;
    return { x: rect.left + rect.width/2, y: rect.top + rect.height/2, element: target.element };
}"""
            ...
            # Focus textarea
            try:
                self.page.evaluate("(cx,cy) => { const el = document.elementFromPoint(cx, cy); if(el && el.tagName && el.tagName.toLowerCase() === 'textarea') { el.focus(); return true; } const t = document.querySelector('textarea[aria-label=Description]'); if(t){ t.focus(); return true; } return false; }", x, y)
            except Exception:
                pass
            ...
            try:
                ...
                print('[DELETE_ALL] textarea became active')
            except Exception:
    ...
    def _do_delete_50(self):
        """Delete last 50 characters from description."""
        try:
            print('[DELETE_50] Starting...')
    ...
    return { x: rect.left + rect.width/2, y: rect.top + rect.height/2, currentValue: target.value };
            ...
            result = self.page.evaluate(js_find)
            if not result:
                print('[DELETE_50] FAILED - No textarea found')
                return
            ...
            print(f'[DELETE_50] Textarea at ({x}, {y}), current value length: {len(current)}')
            ...
            try:
                ...
                print('[DELETE_50] textarea became active')
            except Exception:
                print('[DELETE_50] WARNING: textarea did not become active within timeout')
            
            # Delete 50 chars from end
            ...
            self._last_description = current[:new_length]
            
        except Exception as e:
            print(f'[DELETE_50] ERROR: {e}')
            import traceback
            traceback.print_exc()
    ...
    def delete_50_chars(self):
        """Queue delete 50 chars command."""
        if not self._running:
            raise RuntimeError('Browser not running')
        self._cmd_queue.put(('delete_50', None))
