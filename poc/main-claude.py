...
class BrowserController:
        ...
        try:
            ...
            while self._running:
                ...
                if cmd == 'stop':
                    ...
                    try:
                        ev, res_container = arg
                        # First, clear all sampling markers to force fresh read
                        with self._state_lock:
                            self._desc_sampled_for_url = None
                            self._desc_sampled_for_photo = None
                            self._last_description = None
                        print('force_sample: cleared all cached state, sampling fresh...')
                        ...
                        print(f"force_sample: final result -> {repr(desc)[:300]}")
                        ev.set()
                    except Exception as e:
                ...
                # Update state after processing command
                self._update_state()
    ...
    def _do_next(self):
        ...
        try:
            ...
            if _changed(before, after):
                # Wait for the info panel to fully load with new photo data
                ...
                # Force immediate sample of new description
                try:
                    print('_do_next: sampling new description after navigation...')
                    attempts = 8
                    for i in range(attempts):
                        photo_src, desc = self._sample_description()
                        print(f"_do_next: sample attempt {i+1}/{attempts} -> desc={repr(desc)[:200]}")
                        if desc:
                            with self._state_lock:
                                self._last_description = desc
                                if photo_src:
                                ...
                                try:
                                    ...
                                    pass
                            print(f'_do_next: successfully sampled new description: {repr(desc)[:240]}')
                            break
    ...
    for (const s of descSelectors) {
        try {
            ...
            for (const e of elements) {
                ...
                console.log('JS: Element text:', text.substring(0, 100));
                
                if (text && text !== 'Add a description' && text.length > 0) {
                    console.log('JS: SUCCESS - Found description:', text.substring(0, 100));
                    return {
    ...
    return {photo_src: best ? best.src : null, selected: null, selector: null, method: 'none'};
}"""
            ...
            print(f"_sample_description: selector={selector}, method={method}")
